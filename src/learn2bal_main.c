...
int main(void) {
	...
	// configure TCE0 for PWM
	TCE0.CTRLA = 0x00 | TC_CLKSEL_DIV1_gc;   	// prescaler = 1; 16MHz/1 = 16MHz
	TCE0.CTRLB = 0x00 | TC0_CCBEN_bm | TC0_CCAEN_bm | TC_WGMODE_SS_gc; 	// normal operation (expire at PER) with OC on A and B channels (PE0, PE1)
	TCE0.CTRLC = 0x00;
	TCE0.CTRLD = 0x00;
	TCE0.CTRLE = 0x00;
	...
	PORTF.DIR |= (PIN3_bm | PIN4_bm | PIN6_bm | PIN7_bm);
	PORTF.OUT &= ~PIN3_bm;
	PORTF.OUT &= ~PIN4_bm;
	PORTF.OUT &= ~PIN6_bm;
	PORTF.OUT &= ~PIN7_bm;
...
// automatically cleared upon ISR execution
ISR(TCC1_OVF_vect){
